angular.module('myApp2',[
    'ngRoute','ngAnimate','ui.bootstrap', 'myFilters','ngCookies',
    'controllers','BlockChainAdminController','HomeController',
    'myServices', 'myDirectives','ngMessages' ,'BillService'])
...
.controller('testController',['$scope','$rootScope','$http','$location','$interval','getTransferByIdService','$window','$timeout','$q',function($scope,$rootScope,$http,$location,$interval,getTransferByIdService,$window,$timeout,$q){
	$scope.testApi=function(){
		$('.data-service-top').slideDown(1000,function(){
			$('#div2 .data-service-return pre').slideUp()
			$http({
				  method:'POST',
				  url:'testApi.do',
				  data:$scope.testBill
				  ...
				  console.log(data);
				  $scope.txid=data;
				  $('#div2 .data-service-msg').slideDown();
			  });
		});
  ...
  $scope.getBytxid=function(){
	 ...
	 var stop=$interval(function(){
		  getTransferByIdService.getTransfer($scope.txid).then(function(data){
			  console.log(data);
			  $scope.txidInfo="正在投票中";
			  $('#div2 .data-service-return pre').slideDown()
			  if(data.id){
				  $scope.txidInfo=angular.toJson(data.transaction.data.payload);
...
.controller('ApiCtrl',function($scope,$location,$anchorScroll){
	...
	$(window).scroll(function(){
		...
		if($scope.scroll>=0 && $scope.scroll<285){
			$('.apiWrap li').eq(0).addClass('active').siblings().removeClass('active');
		}else if($scope.scroll>=285 && $scope.scroll<590){
			$('.apiWrap li').eq(1).addClass('active').siblings().removeClass('active');
		}else if($scope.scroll>=590 && $scope.scroll<895){
			$('.apiWrap li').eq(2).addClass('active').siblings().removeClass('active');
		}else if($scope.scroll>=895 && $scope.scroll<1200){
			$('.apiWrap li').eq(3).addClass('active').siblings().removeClass('active');
		}else if($scope.scroll>=1200 && $scope.scroll<1505){
			$('.apiWrap li').eq(4).addClass('active').siblings().removeClass('active');
		}else if($scope.scroll>=1505 && $scope.scroll<1810){
			$('.apiWrap li').eq(5).addClass('active').siblings().removeClass('active');
		}else if($scope.scroll>=1810 && $scope.scroll<2137){
			$('.apiWrap li').eq(6).addClass('active').siblings().removeClass('active');
		}else if($scope.scroll>=2137 && $scope.scroll<2465){
			$('.apiWrap li').eq(7).addClass('active').siblings().removeClass('active');
		}else if($scope.scroll>=2465 && $scope.scroll<2665){
			$('.apiWrap li').eq(8).addClass('active').siblings().removeClass('active');
		}else{
			$('.apiWrap li').eq(9).addClass('active').siblings().removeClass('active');
		}
		/*if($scope.scroll>=96){
			$('.apiWrap ul').css('top','90px');
		}else{
			$('.apiWrap ul').css('top','180px');
		}*/
	})
}).controller('LoginController',['$scope','$rootScope','$location','$window','$cookies','$cookieStore','loginServices',function($scope,$rootScope,$location,$window,$cookies,$cookieStore,loginServices){
	//首页登陆控制器，四种登陆角色 开发者、节点用户、区块链管理员、超级管理员
	//用来存放发送、接受的用户信息
	
	$scope.adminLogin=function(){
		loginServices.login('adminLogin',$scope.postUser).then(function(data){
...
//			}
	...
	$scope.bcaLogin=function(){
		loginServices.login('blockChainAdminLogin',$scope.postUser).then(function(data){
			console.log($scope.postUser);
			console.log(data);
//			if($scope.postUser.referrerKey!=data.referrerKey){
//				Showbo.Msg.alert("您的秘钥或密码有错！");
//			}
	...
	$scope.devLogin=function(){
		loginServices.login('xxxxx',$scope.postUser).then(function(data){
			console.log($scope.postUser);
			console.log(data);
//			if($scope.postUser.referrerKey!=data.referrerKey){
//				Showbo.Msg.alert("您的秘钥或密码有错！");
