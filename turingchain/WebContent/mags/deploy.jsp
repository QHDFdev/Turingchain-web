          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tbody>
                <c:forEach var="nodeDep" items="${noderDeploy}">
                <tr>
                  <td>${nodeDep.nodeIP}</td>
                   <td>${nodeDep.nodeName}</td>
                  <td>${nodeDep.nodeID}</td>
                  <td>${nodeDep.region}</td>
                  <td>${nodeDep.ownedOrgnization}</td>
                  <td>${nodeDep.respPerson}</td>
                  <td>${nodeDep.nodeType}</td>
                  <%-- <td>${nodeDep.nodePUBKEY}</td>
                  <td>${nodeDep.createdTime}</td>
                  <td>${nodeDep.runtime}</td>
                  <td>${nodeDep.runStatistics}</td> --%>
                   <%-- <td> 
                    	<button  onclick="javascript:if(confirm('确实要删除吗?'))location.href='deleteById/${nodeDep.nodeID}.do'" type="button" class="btn btn-danger btn-sm">
      						删除
   						</button>
   				  </td> --%>
				  <td>
                     <a href="<%=request.getContextPath()%>/admin/nodeDeployinfo/${nodeDep.nodeID}.do" >
