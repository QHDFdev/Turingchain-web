          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tbody>
                <c:forEach var="bcAdmin" items="${blockChainAdmin}">
                <tr>
                  <td>${bcAdmin.bcAdmName}</td>
                  <td>${bcAdmin.email}</td>
                  <td>${bcAdmin.registDate}</td>
                  <td>${bcAdmin.blockChainAdminState}</td>
                  <td>
                    <a href="<%=request.getContextPath()%>/getBlockChainAdminInfoById2/${bcAdmin.id}.do"><button type="button" class="btn btn-block btn-info" >审核信息</button></a>
                  </td>
                </tr>
                </c:forEach>
              </table>
 ...
 <script type="text/javascript">
	$(function() {
		...
		$(".delete").click(function() {
			var flag = Showbo.Msg.confirm("nodersAll确定删除该节点用户?");
			if (flag) {
				...
				$.get(url, args, function(data) {
					...
					if (data == "1") {
						...
						Showbo.Msg.alert("nodersAll删除失败!");
					}
				});
			}
			//取消超链接的默认行为
